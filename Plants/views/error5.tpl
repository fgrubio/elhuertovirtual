...
<header>
  <div class="topnav">
    ...
    <b href="">Mi Huerto</b>
    <c href="">|</c>
    <c href="">Error al editar tipo {{.planta.Tipo}}</c>
  </div>
</header>
<body>
  <div class="div1"></div>
  <h2> Estás intentando cambiar a tipo {{.planta.Tipo}}, cuando ya se está cultivando en el huerto. </h2>
  <h2> Puede hacer lo siguiente: </h2>
  </p>
  <br><br>
  <div align="center">
    <input type="button" onclick="editar({{.planta.ID}})" value="Canviarle tipo" class="boton">
    <input type="button" onclick="eliminar({{.planta.ID}})" value="Eliminar el original y ponerle los valores nuevos" class="boton">
...
<script>
  ...
  function editar(s) {
    location.href= "/edit?key=" + {{.plantanueva.ID}} +
    "&tipo="+ {{.plantanueva.Tipo}} + 
    "&cantidad="+ {{.plantanueva.Cantidad}} +
    "&duracion="+ {{.plantanueva.Duracion}} +
    "&seleccio="+ {{.plantanueva.Seleccio}};
